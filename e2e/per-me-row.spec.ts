...
// solo l'ordine. «Attaccante Scarso» a 2 cr è il caso che ammazzava la strada
// precedente — con la sottrazione `valore − ancora` sarebbe l'unico candidato
// positivo, cioè il primo, cioè il peggiore promosso a occasione. Qui è ultimo,
// e a tetto di tre righe non compare affatto.
...
test("il sottoblocco mostra i candidati attesi, in ordine, dentro GIOCATORE SUGGERITO", async ({
  ...
  // I CRITERI, DETTI A SCHERMO. La posizione è una posizione nell'ordine del
...
test.describe("col dito, a 390x844", () => {
  ...
  test("il tocco fa esattamente quello che fa il clic, fino alla schermata d'asta", async ({
    ...
    //
    // Il numero, su questa scena e a 390x844: 574px il sottoblocco pieno
    // (615px con l'occhiello «GIOCATORE SUGGERITO»), 120px quello muto (161px
    // con l'occhiello). Il segnaposto che stava qui prima — stesso stile,
    // stesso testo, rimesso a schermo e misurato invece che ricordato — ne
    // costava 98 (139 con l'occhiello). Il costo NETTO di questa tranche è
    // quindi +476px nello stato pieno e +22px in quello muto.
    //
