...
// #59 — IL BUDGET VERTICALE HA UN TOTALE E NESSUN PROPRIETARIO: L'ULTIMO
...
test("PROVA 1 — una riga di testo in più a un blocco esistente: rosso il mastro col suo nome, verde il totale", async ({
  ...
  await page.evaluate(() => {
    ...
    extra.style.cssText = "font-size:13px;line-height:1.5;";
  ...
  expect(findings[0]).toMatchObject({ kind: "oltre-allocazione", id: "giocatore-suggerito" });
